...
INPUT_PATH = SCRIPT_DIR.parent / "data" / "ads" / "ads.xlsx"
OUTPUT_PREFIX = SCRIPT_DIR.parent / "data" / "ads" / "employer_branding" / "ads_labeled.xlsx"

# Column names
COL_TEXT = "snapshot/body/text"  # Primary text column for ads
COL_TEXT_FALLBACK = "snapshot/body"  # Fallback text column
...
EMPLOYER_BRANDING_SYSTEM_PROMPT = (
    "You are a classifier for Facebook/Instagram ads.\n"
    "Determine if an ad is about employer branding or related to showing what it's like to work at the company.\n"
    "Label as 1 (employer branding) if the ad:\n"
    ...
    "Label as 0 (not employer branding) if the ad:\n"
...
THEME_SYSTEM_PROMPT = (
    "You are labeling Facebook/Instagram ads against a FIXED taxonomy of employer branding themes.\n"
    "Rules:\n"
    "- Choose 1 to 3 themes from ALLOWED THEMES (listed below) that best describe the ad. Don't force yourself to choose more than 1 theme if it does not fit any of the other themes.\n"
    ...
    "14. OTHER — Ad does not fit any of the above themes\n"
)
...
def build_employer_branding_prompt(ad_text: str) -> str:
    return f"Facebook/Instagram ad:\n{ad_text}"

def build_theme_prompt(ad_text: str) -> str:
...
def generate_employer_branding_label(client: OpenAI, model: str, ad_text: str) -> int:
    ...
    try:
        resp = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": build_employer_branding_prompt(ad_text)},
            ],
...
def generate_themes(client: OpenAI, model: str, ad_text: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    """Generate theme labels: up to 3 themes per ad."""
    try:
        resp = client.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": THEME_SYSTEM_PROMPT},
                {"role": "user", "content": build_theme_prompt(ad_text)},
            ],
            temperature=TEMPERATURE,
            max_tokens=150,
        )
...
def process_single_ad(
    ...
    """
    Process a single ad and generate labels based on enabled features.
...
def label_dataframe(
    ...
    if COL_TEXT in df.columns:
        text_col = COL_TEXT
    elif COL_TEXT_FALLBACK in df.columns:
        text_col = COL_TEXT_FALLBACK
    else:
        raise ValueError(f"Input file is missing required text column. Expected '{COL_TEXT}' or '{COL_TEXT_FALLBACK}'")

    # Keep only non-empty texts
    df = df.copy()
    df[text_col] = df[text_col].map(lambda x: compact_text(x if isinstance(x, str) else ""))
