...
def _load_archetype_stats_from_agility():
    """Load archetype statistics from Agility data (PR data source)."""
    stats = {}
    for brand in BRANDS:
...
def _render_archetype_matrix(counts_dict, total_count, item_label="press releases"):
...
def render():
    ...
    if not archetype_stats:
        st.info("No archetype data available. Ensure Agility files contain 'Top Archetype' column.")
        return
    ...
    with matrix_tabs[0]:
        ...
        _render_archetype_matrix(overall_counts, overall_total, "press releases")
    ...
    for idx, (company, info) in enumerate(archetype_stats.items()):
        with matrix_tabs[idx + 1]:
            ...
            _render_archetype_matrix(info.get("counts", {}), info.get("total", 0), "press releases")
