...
def _load_employer_branding_themes_data():
    """Load employer branding themes data from ads_labeled.xlsx and return dict of company -> list of top 5 themes with percentages and examples."""
    ...
    try:
        ...
        # Check for text column (ads use snapshot/body/text or snapshot/body)
        text_col = None
        ...
        if company_col is None or 'theme_1' not in df.columns or text_col is None:
            return {}
        ...
        for company in df[company_col].unique():
            ...
            # Normalize company name using the same method as ads dashboard
            ...
            for theme, count in theme_counts.most_common(5):
                ...
                examples = []
                # Check all theme columns to find ads with this theme
                for theme_col in ['theme_1', 'theme_2', 'theme_3']:
                    ...
                    theme_df = theme_df[theme_df[text_col].notna()].copy()
                    ...
                    for _, row in theme_df.iterrows():
                        ...
                        if ad_text not in used_ad_texts:
                            ...
                            if len(ad_text) > 150:
                                ad_text_display = ad_text[:150] + "..."
                            else:
                                ad_text_display = ad_text
                            
                            # Get adArchiveID for the link if available
                            ad_id = None
...
def _load_theme_distribution_data():
    ...
    try:
        ...
        # For each theme, count ads per company (checking all 3 theme columns)
        theme_distribution = {}
...
def _render_theme_distribution_charts():
    ...
    for idx, theme in enumerate(all_themes):
        with tabs[idx]:
            ...
            chart_df = chart_df.sort_values('Ads', ascending=False)
...
def _render_company_theme_stacked_bar():
    """Render stacked bar chart showing number of ads per company, stacked by theme."""
    ...
    st.markdown("Stacked bar chart showing total ads per company, divided by theme.")
    ...
    # Sort companies by total ads (descending)
    ...
    fig.update_layout(
        barmode='stack',
        title='Number of Employer Branding Ads by Company and Theme',
        xaxis_title='Company',
    ...
    fig.update_traces(
        hovertemplate='<b>%{fullData.name}</b><br>Company: %{x}<br>Ads: %{y}<extra></extra>'
    )
